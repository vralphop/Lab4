...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void printArray( )
   {
	...
	// Note:  To animate the algorithm, put this method call as the
	   ...
	   for (int i = 0; i < intArray.length; i++ )
	   {
		   int []arrayRow = intArray[i];
		   System.out.println(" ");
		   System.out.print(i + "\t");
		   for (int j = 0; j < arrayRow.length; j++ )
		   {
			   int arrayCol = arrayRow[j];
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    //  and column is the index of the array's current column
    // Write your code here:
	   for (int k = 0; k < intArray[row].length; k++)
	   {
		   intArray[row][k] = value;
		   animate (row, k);
	   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   	int min = -1, arrayRow = 0;
	   	
	   	for (int i = 0; i < intArray.length; i++)
	   	{
	   		int value  = intArray[i][column];
	   		if (min == -1 || value < min)
	   		{
	   			min = value; 
	   			arrayRow = i;
